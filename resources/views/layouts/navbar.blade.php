        ...
        <div :class="{'block': mobileMenuOpen, 'hidden': !mobileMenuOpen}"
            ...
            <ul
                ...
                <li><a href="{{ route('home') }}"
                        class="block py-2 px-3 rounded md:p-0 {{ request()->routeIs('home') ? 'text-sky-500' : 'text-gray-400 hover:text-white' }}"
                        aria-current="page">Beranda</a></li>

                <li x-data="{ open: false }" class="relative">
                    <button @click="open = !open" @click.away="open = false"
                        class="flex items-center justify-between w-full py-2 px-3 text-gray-400 hover:text-white md:p-0 md:w-auto">
                        Profil <i class="fa-solid fa-chevron-down fa-xs ml-1"></i>
                    </button>
                    <div x-show="open" x-transition class="absolute mt-2 w-48 bg-white rounded-md shadow-lg z-20">
                        @forelse ($profilPages as $page)
                            <a href="{{ route('halaman.show', $page) }}"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">{{ $page->judul }}</a>
                        @empty
                            <span class="block px-4 py-2 text-sm text-gray-500">Kosong</span>
                        @endforelse
                    </div>
                </li>

                <li><a href="{{ route('berita.index') }}"
                        class="block py-2 px-3 rounded md:p-0 {{ request()->routeIs('berita.*') ? 'text-sky-500' : 'text-gray-400 hover:text-white' }}">Berita</a>
                </li>
                <li><a href="{{ route('agenda.index') }}"
                        class="block py-2 px-3 rounded md:p-0 {{ request()->routeIs('agenda.*') ? 'text-sky-500' : 'text-gray-400 hover:text-white' }}">Agenda</a>
                </li>
                <li><a href="{{ route('dokumen.index') }}"
                        class="block py-2 px-3 rounded md:p-0 {{ request()->routeIs('dokumen.*') ? 'text-sky-500' : 'text-gray-400 hover:text-white' }}">Dokumen</a>
                </li>
            </ul>
        </div>
    </div>
